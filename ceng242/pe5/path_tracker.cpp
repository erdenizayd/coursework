...
PathTracker::PathTracker(int *existing_path, int number_of_vectors)
{
    ...
    for(int i = 0; i < number_of_vectors; i++){
        final_x = final_x + existing_path[2*i];
        final_y = final_y + existing_path[2*i + 1];
        displacement = calculate_displacement();
        calculate_distance(existing_path[2*i], existing_path[(2*i) + 1]);
    }
}
...
{
    distance = distance + sqrt(pow(x,2) + pow(y,2));
}
