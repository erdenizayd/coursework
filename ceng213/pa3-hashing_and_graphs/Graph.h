...
private:
    ...
    int _edge;
    int firstkey;
    void deleteHelper(int, int);
    bool cyclicHelper(int, HashTable<int,bool> &, HashTable<int,bool> &);
    ...
    struct Pair{
        ...
        Pair(int k, long d, const Node* n, const Node* p){
            key = k;
            distance = d;
            node = n;
            prev = p;
    ...
    {
        public:
            int operator() (Graph::Pair& pair1, Graph::Pair& pair2)
            {
